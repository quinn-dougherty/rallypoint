...
export default async function Page() {
    ...
    "use server";
    const displayName = formData.get("displayName") as string||data?.display_name;
    const lwUsername = formData.get("lwUsername") as string || data?.lw_username;
    ...
    if(lwUsername!==null && lwUsername!==data?.lw_username){
     ...
     await UpdateUser({user_id:user.id,displayName,lwUsername,bio}).then(
         redirect("/profile")
     ).catch((error)=>{
